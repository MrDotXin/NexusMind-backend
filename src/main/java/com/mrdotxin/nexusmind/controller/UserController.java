...
import static com.mrdotxin.nexusmind.service.impl.UserServiceImpl.SALT;
...
public class UserController {
    ...
    public BaseResponse<String> uploadUserAvatarByMultipart(@RequestPart("file") MultipartFile file, @RequestParam("userId") Long userId, HttpServletRequest httpServletRequest) {
        ThrowUtils.throwIf(ObjectUtil.isNull(file), ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(ObjectUtil.isNull(userId) || userId == 0, ErrorCode.PARAMS_ERROR);

        User user = userService.getLoginUser(httpServletRequest);
        String avatar = userService.uploadUserAvatar(FileUploadTypeEnum.MULTIPART_FILE.getValue(), file, user);

        return ResultUtils.success(avatar);
    }

    @PostMapping("/upload/avatar/url")
    public BaseResponse<String> uploadUserAvatarByUrl(@RequestParam("url") String fileUrl, @RequestBody FileUploadRequest fileUploadRequest, HttpServletRequest httpServletRequest) {
        ...
        User user = userService.getLoginUser(httpServletRequest);
        String avatar = userService.uploadUserAvatar(FileUploadTypeEnum.URL.getValue(), fileUrl, user);
